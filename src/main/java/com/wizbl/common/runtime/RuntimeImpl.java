...
import com.wizbl.common.runtime.vm.program.Program.JVMStackOverFlowException;
import com.wizbl.common.runtime.vm.program.Program.OutOfTimeException;
import com.wizbl.common.runtime.vm.program.ProgramPrecompile;
...
public class RuntimeImpl implements Runtime {
  ...
  private final VMConfig config = VMConfig.getInstance();
  ...
  /**
   * trxType에 맞춰서 precompiled(), create(), call() 메소드를 호출함.
  ...
  public long getTotalEnergyLimitWithFixRatio(AccountCapsule creator, AccountCapsule caller,
    ...
    if (Arrays.equals(creator.getAddress().toByteArray(), caller.getAddress().toByteArray())) {
      ...
      // which will cost the feeLimit WBL when the creator's frozen energy is 0.
      return callerEnergyLimit;
    }
  ...
  public boolean isCallConstant() throws ContractValidateException {
    ...
    TriggerSmartContract triggerContractFromTransaction = ContractCapsule.getTriggerContractFromTransaction(trx);
    if (TrxType.TRX_CONTRACT_CALL_TYPE == trxType) {
      ...
      return Wallet.isConstant(abi, triggerContractFromTransaction);
    }
    return false;
  }
  ...
  private boolean isCallConstant(byte[] address) throws ContractValidateException {
    ...
    if (TrxType.TRX_CONTRACT_CALL_TYPE == trxType) {
      ...
      return Wallet.isConstant(abi, ContractCapsule.getTriggerContractFromTransaction(trx));
    }
    return false;
  }
  ...
  public void finalization() {
    if (StringUtils.isEmpty(runtimeError)) {
      for (DataWord contract : result.getDeleteAccounts()) {
        deposit.deleteContract(convertToBrte2Address((contract.getLast20Bytes())));
      }
    }
