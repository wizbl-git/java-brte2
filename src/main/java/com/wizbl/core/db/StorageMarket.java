...
public class StorageMarket {
  ...
  private long exchange_to_supply(boolean isWBL, long quant) {
    logger.info("isWBL: " + isWBL);
    long balance = isWBL ? dbManager.getDynamicPropertiesStore().getTotalStoragePool() :
...
//    }
  ...
  private long exchange_to_supply2(boolean isWBL, long quant) {
    logger.info("isWBL: " + isWBL);
    long balance = isWBL ? dbManager.getDynamicPropertiesStore().getTotalStoragePool() :
        dbManager.getDynamicPropertiesStore().getTotalStorageReserved();
    logger.info("balance: " + balance);
...
//    }
  ...
  private long exchange_from_supply(boolean isWBL, long supplyQuant) {
    long balance = isWBL ? dbManager.getDynamicPropertiesStore().getTotalStoragePool() :
        dbManager.getDynamicPropertiesStore().getTotalStorageReserved();
    supply -= supplyQuant;
  ...
  public long exchange(long from, boolean isWBL) {
    long relay = exchange_to_supply(isWBL, from);
    return exchange_from_supply(!isWBL, relay);
  }
