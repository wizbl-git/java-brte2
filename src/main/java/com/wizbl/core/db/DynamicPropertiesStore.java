...
import com.wizbl.core.config.Parameter;
...
/**
 * DynamicPropertiesStore 클래스는 블록체인에서 사용되는 최신(latest)의 정보를 저장하고 있는 store임. <br/>
 *  - 특정 작업(ex. generateBlock)이 시작되거나 종료할 때 이 store의 값을 참조하거나, 이 store 내부의 항목 값을 갱신하는 작업이 발생함. <br/>
 */
@Slf4j
@Component
public class DynamicPropertiesStore extends Brte2StoreWithRevoking<BytesCapsule> {
  ...
  private static final byte[] MANA_FEE = "MANA_FEE".getBytes();
  private static final byte[] FREE_MANA_LIMIT = "FREE_MANA_LIMIT".getBytes();
  ...
  private DynamicPropertiesStore(@Value("properties") String dbName) {
    ...
    try {
      ...
      this.saveAccountUpgradeCost(999_900_000_000L);    // x 100    Because Change unit (1,000,000 -> 100,000,000)
    }
    ...
    try {
      this.getFreeManaLimit();
    } catch (IllegalArgumentException e) {
      this.saveFreeManaLimit(5_000L);
    }
    ...
    try {
      ...
      this.saveCreateAccountFee(0L); // 0 WBL
    }
    ...
    try {
      ...
      this.saveTransactionFee(1000L); // 10sun/byte -> 1,000 / byte
    }

    try {
      this.getAssetIssueFee();
    } catch (IllegalArgumentException e) {
      this.saveAssetIssueFee(102_400_000_000L);     // x 100    Because Change unit (1,000,000 -> 100,000,000)
    }

    try {
      this.getExchangeCreateFee();
    } catch (IllegalArgumentException e) {
      this.saveExchangeCreateFee(102_400_000_000L); // x 100    Because Change unit (1,000,000 -> 100,000,000)
    }
  ...
  public void saveManaFee(long totalTestManaLimit) {
    this.put(MANA_FEE,
            new BytesCapsule(ByteArray.fromLong(totalTestManaLimit)));
  }

  public long getManaFee() {
    return Optional.ofNullable(getUnchecked(MANA_FEE))
            .map(BytesCapsule::getData)
            .map(ByteArray::toLong)
            .orElseThrow(
                    () -> new IllegalArgumentException("not found MANA_FEE"));
  }

  public void saveFreeManaLimit(long freeTestManaLimit) {
    this.put(FREE_MANA_LIMIT,
            new BytesCapsule(ByteArray.fromLong(freeTestManaLimit)));
  }

  public long getFreeManaLimit() {
    return Optional.ofNullable(getUnchecked(FREE_MANA_LIMIT))
            .map(BytesCapsule::getData)
            .map(ByteArray::toLong)
            .orElseThrow(
                    () -> new IllegalArgumentException("not found FREE_MANA_LIMIT"));
  }
