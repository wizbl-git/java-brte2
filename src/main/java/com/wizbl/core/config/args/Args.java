...
import com.wizbl.core.config.Configuration;
...
import com.wizbl.keystore.CipherException;
import com.wizbl.keystore.Credentials;
import com.wizbl.keystore.WalletUtils;
import com.wizbl.program.Version;
...
//  private long syncNodeCount;
  ...
  //If you are running a solidity node for java brte2, this flag is set to true
  @Getter
  @Setter
...
//        config.hasPath("sync.node.count") ? config.getLong("sync.node.count") : 0;
    ...
    INSTANCE.maintenanceTimeInterval =
        config.hasPath("block.maintenanceTimeInterval") ? config
            .getInt("block.maintenanceTimeInterval") : 21600000L; // 1000 * 60 * 60 * 6 (6H)
    ...
    /**
     * config.conf에서 smartContract 에있는 주소값을 가져온다. 이값은 SetPrescriptionServlet.java에서 사용한다.
     * 스마트 컨트랙트 주소를 고정한값.
     */
    INSTANCE.contractAddress =
            config.hasPath("smartContract.contractAddress") ? config.getString("smartContract.contractAddress") : "410fca8d03f53a21c072950b33fba7010cf468606b";
    initBackupProperty(config);

    INSTANCE.ownerAddress =
            config.hasPath("smartContract.ownerAddress") ? config.getString("smartContract.ownerAddress") : "4106f51c249260a777ae1b1ce47c61122278894fc6";
    initBackupProperty(config);

    INSTANCE.sPrivateKey =
            config.hasPath("smartContract.sPrivateKey") ? config.getString("smartContract.sPrivateKey") : "af7c83e40cc67a355852b44051fc9e34452375ae569d5c18dd62e3859b9be229";
    initBackupProperty(config);
