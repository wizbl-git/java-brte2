...
import com.wizbl.core.db.VotesStore;
import com.wizbl.core.db.WitnessStore;
import com.wizbl.core.exception.HeaderNotFound;
...
/**
 * WitnessController는 WitnessStore에 저장되어 있는 Witness의 정보를 메모리에서 관리하는 클래스임.
 */
@Slf4j
public class WitnessController {
  ...
  /**
   * manager 객체에 의해서 관리되는 WitnessController 객체를 생성 및 반환함.
   * @param manager
   * @return
   */
  //TODO WitnessController가 프로그램상에서 유일한 인스턴스여야 한다면 코드 수정이 필요해 보임.
  ...
  /**
   * WitnessStore의 Witnesses를 바탕으로 Witness 정보를 초기화함.
   */
  public void initWits() {
    ...
    sortWitness(witnessAddresses);  // vote count에 의해서 재정렬
...
//          account.createReadableString());
  ...
  /**
   * witness list를 vote count에 의해서 재정렬함.
   * @param list
   */
