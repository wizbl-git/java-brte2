...
import com.wizbl.core.db2.common.Key;
import com.wizbl.core.db2.common.Value;
import com.wizbl.core.exception.RevokingStoreIllegalStateException;

/**
 * 블록체인의 DB를 관리하기 위한 Manager 객체 <br/>
 * db.version = 2 인 경우에는 SnapshotManager 객체를,7 1 인 경우에는 AbstractRevokingStore 객체를 사용하여 블록체인 DB를 관리함.
 */
@Slf4j
public class SnapshotManager implements RevokingDatabase {
    ...
    /**
     * Brte2StoreWithRevoking객체에서 생성된 IRevokingDB 구현 객체를 List에 add시킴 <br/>
     * 개별 DB 정보를 ListeningExecutor에 할당함.
     *
     * @param db
     */
    @Override
    public void add(IRevokingDB db) {
        ...
        flushServices.put(revokingDB.getDbName(), MoreExecutors.listeningDecorator(
                Executors.newSingleThreadExecutor(new ThreadFactoryBuilder().setNameFormat(revokingDB.getDbName()).build())));
    }

    /**
     * 기존의 DB Head에 새로 생성한 SnapshotImpl 객체를 set함.
     */
    ...
    /**
     * tmp DB에 저장되어 있는 데이터 가운데 처리되지 않은 데이터를 각 개별 DB에 저장 및
     * 기존에 저장된 데이터와 신규 데이터를 merge 하는 메소드
     */
    @Override
    public void check() {
        for (RevokingDBWithCachingNewValue db : dbs) {
        ...
        tmpLevelDbDataSource = new LevelDbDataSourceImpl(Args.getInstance().getOutputDirectoryByDbName("tmp"), "tmp");
        tmpLevelDbDataSource.initDB();
        // tmp DB에 기존에 처리되지 않은 데이터가 존재하는 경우에만 실행됨.
        if (!tmpLevelDbDataSource.allKeys().isEmpty()) {
    ...
    /**
     * 바이트 배열을 decoding하여 String값을 추출한 후 반환함. <br/>
     * //TODO 바이트 배열을 문자열로 변환하는 작업에 대한 방식은 분석이 필요함.
     *
     * @param bytes
     * @return
     */
