...
import com.wizbl.protos.Contract.AssetIssueContract.FrozenSupply;
...
public class AssetIssueActuatorTest {
    ...
    private static final String URL = "acorninc.net";
    ...
    public void SameTokenNameCloseAssetIssueSuccess() {
        ...
        Long blackholeBalance = dbManager.getAccountStore().getSquirrel().getBalance();
        try {
            actuator.validate();
            ...
            Assert.assertEquals(dbManager.getAccountStore().getSquirrel().getBalance(),
    ...
    public void oldNotUpdateAssetIssueSuccess() {
        ...
        try {
            ...
            Assert.assertFalse(dbManager.getAssetIssueStore().has(ByteString.copyFromUtf8(NAME).toByteArray()));
            // check V2
            long tokenIdNum = dbManager.getDynamicPropertiesStore().getTokenIdNum();
            AssetIssueCapsule assetIssueCapsuleV2 =
                    dbManager.getAssetIssueV2Store().get(ByteArray.fromString(String.valueOf(tokenIdNum)));
   ...
   */
    ...
    public void assetNameTest() {
        ...
        blackholeBalance = dbManager.getAccountStore().getSquirrel().getBalance();
        try {
            actuator.validate();
            actuator.execute(ret);
            Assert.assertTrue(false);
    ...
    public void frozenListSizeTest() {
        ...
        for (int i = 0; i < dbManager.getDynamicPropertiesStore().getMaxFrozenSupplyNumber() + 2;
             i++) {
