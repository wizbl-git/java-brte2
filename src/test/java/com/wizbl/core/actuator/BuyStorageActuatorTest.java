...
//  }
    ...
    public void buyLessThan1Trx() {
        ...
        try {
            ...
            Assert.assertTrue(e instanceof ContractValidateException);
            Assert.assertEquals("quantity must be larger than 1WBL", e.getMessage());
        } catch (ContractExeException e) {
            Assert.assertFalse(e instanceof ContractExeException);
        }
    }
    ...
    public void invalidOwnerAccount() {
        long quant = 1_000_000_000L;
        BuyStorageActuator actuator = new BuyStorageActuator(getContract(OWNER_ACCOUNT_INVALID, quant), dbManager);
        TransactionResultCapsule ret = new TransactionResultCapsule();
        try {
            actuator.validate();
            actuator.execute(ret);
